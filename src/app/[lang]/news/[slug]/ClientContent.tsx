...
const ClientContent = ({ gallery, files, currentLang, tags }: BlogPageContent) => {
    ...
    <article className='w-full max-w-[750px] mx-auto'>
      <div ref={emblaRef} className='w-full embla  lg:-mt--desktop---2xl md:-mt--tablet---2xl -mt--mobile---2xl'>
        ...
        <div className='w-full px-4'>
          ...
          {tags && (
            <div className='w-full max-w-[750px] mx-auto flex items-center justify-start gap-2 flex-wrap lg:-mt--desktop---2xl md:-mt--tablet---2xl -mt--mobile---2xl'>
              {tags.edges.map((t: any) => {
                ...
                return (
                  <a
                    key={t.node.id}
                    href={`/${currentLang}/news?tag=${t.node.name.replace('#', '')}`}
